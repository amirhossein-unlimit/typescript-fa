...
**روش دوم بررسی نوع متغیر:** این روش تعیین می‌کند که یک متغیر، type مشخصی دارد.
...
برای تشخیص نوع شکل از یک فیلد به نام `kind` استفاده می‌کنیم.
...
function handleStatus(status: Status) {
  if (status === 'loading') { /* ... */ } else if (status === 'success') { /* ... */ } else if (status === 'error') { /* ... */ } else {
